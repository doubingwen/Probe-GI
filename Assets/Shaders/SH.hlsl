    ...
    )
{
    ...
    float3 normalWeights[8] = { float3(0, 0, 0), float3(0, 0, 0), float3(0, 0, 0), float3(0, 0, 0), float3(0, 0, 0), float3(0, 0, 0), float3(0, 0, 0), float3(0, 0, 0), };
    ...
    for(int i=0; i<8; i++)
    {
        ...
        normalWeights[i] = normalWeight.xxx;
    ...
    float3 rate = saturate((worldPos.xyz - minCorner) / _coefficientVoxelGridSize);
    float interpolatedWeight = TrilinearInterpolationFloat3(normalWeights, rate).x;
    float3 color = TrilinearInterpolationFloat3(Lo, rate) / max(interpolatedWeight, 0.0005);
